...
def train(args):
    model = Emuru(args.t5_checkpoint, args.vae_checkpoint, args.slices_per_query).to(args.device)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for i, batch in tqdm(enumerate(loader), total=len(loader), desc=f'Epoch {epoch}'):
            ...
            # imgs = model.generate(input_ids=batch['input_ids'], img=batch['img'], max_new_tokens=96 - 16, decoder_truncate=16)
