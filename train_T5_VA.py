...
class Emuru(torch.nn.Module):
    def __init__(self,
        ...
        with open(Path('files/unifont.pickle'), 'rb') as f:
            self.archetypes = pickle.load(f)
        self.archetypes = torch.stack([torch.tensor(a['mat']) for a in self.archetypes]).flatten(1).float()

        self.tokenizer_to_unifont_map = torch.ones(len(self.tokenizer), dtype=torch.long) * ord(' ')
        for input_id in range(len(self.tokenizer)):
            if input_id in self.tokenizer.all_special_ids:
                continue
            unifont_id = self.tokenizer.decode(input_id)
            if len(unifont_id) != 1:
                continue
            self.tokenizer_to_unifont_map[input_id] = ord(unifont_id)
        ...
        self.archetypes_emb = torch.nn.Linear(16 * 16, config.d_model)
    ...
    def ids_to_embed(self, input_ids):
        device = input_ids.device
        self.tokenizer_to_unifont_map = self.tokenizer_to_unifont_map.to(device)
        self.archetypes = self.archetypes.to(device)

        unifont_ids = self.tokenizer_to_unifont_map[input_ids]
        return self.archetypes_emb(self.archetypes[unifont_ids])
    ...
    def forward(self, text=None, img=None, input_ids=None, attention_mask=None, length=None, noise=0):
        ...
        inputs_embeds = self.ids_to_embed(input_ids)
        output = self.T5(inputs_embeds=inputs_embeds, attention_mask=attention_mask, decoder_inputs_embeds=decoder_inputs_embeds)
    ...
    def generate(self, text=None, img=None, input_ids=None, max_new_tokens=96, decoder_truncate=None):
        ...
        for _ in range(max_new_tokens):
            ...
            inputs_embeds = self.ids_to_embed(input_ids)
            output = self.T5(inputs_embeds=inputs_embeds, decoder_inputs_embeds=decoder_inputs_embeds)
            vae_latent = self.t5_to_vae(output.logits[:, -1:])
    ...
    def save_pretrained(self, path):
        ...
        torch.save(self.archetypes_emb.state_dict(), path / 'archetypes_emb.pth')
    ...
    def load_pretrained(self, path):
        ...
        self.archetypes_emb.load_state_dict(torch.load(path / 'archetypes_emb.pth'))
